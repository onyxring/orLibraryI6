...
! BEFORE  PARSER
    ...
    default orStringDefaultSize	1000;  !we use big strings to capture and analyze game behavior
...
! AFTER PARSER
    ...
    orInfExt with  ext_displayStatus[; !--each time a turn is complete, give control to the Testing Framework to see what needs to be done
            ...
            ext_initialise[c;
              if(orUnitTestAutoRun==false) return;
...
!---------------------------------------------------------------------------------------------------
! utSuite01.inf
! A unit test suite to test several orLibrary extensions.  These unit tests are, themselves, 
! formatted as extensions.
! This suite of test covers the following extensions:
!
!   orAdjective
!---------------------------------------------------------------------------------------------------
! Created by Jim Fisher
!---------------------------------------------------------------------------------------------------
! 2024.05.04	Initial revision
!---------------------------------------------------------------------------------------------------
Constant Story				"orLibrary UnitTest Suite 1";
Constant Headline			"^Copyright (c) 2024: Jim Fisher^";
Release 1;
!---------------------------------------------------------------------------------------------------
! Include orLibrary Extensions here
!---------------------------------------------------------------------------------------------------
!#include "utAdjective";
! #include "utBetterChoice";
! #include "utPrefixSuffix";
! #include "utDistinctMeSelf";
! #include "utRecogName";
!#include "utDoor";
!#include "utNameable";
!#include "utParentChildRelationship";
!#include "orTest";
!#include "utReferByContents";
#include "orFirstImpressions";
#include "_orUnitTest";
!---------------------------------------------------------------------------------------------------
constant orStringDefaultSize 2000;
#include "#parser";
!---------------------------------------------------------------------------------------------------
#include "#verblib"; 
object startObj "orLib Test Suite" has light with description "The orLibrary Test Suite.";



object candlestick "candlestick" startObj
...
orUnitTest with setup[;
      playerCommands.pushCommand("x candlestick");
    ] 
    [;
      playerCommands.pushCommand("x candlestick");
    ]
  , assess[str; 
      print "^Contains ~Definitely lethal~: ";
      str.print();
      if(str.contains("Definitely lethal.")) {
        print "Pass.";
        rtrue;
      }
      else{
        print "FAILED.^"; testResult.print();
      }
    ]
    [str; 
      print "^Does not contains ~Definitely lethal~: ";
      if(str.contains("Definitely lethal.")) {
        print "FAILED.^"; testResult.print();
      }
      else{
