...
! AFTER VERBLIB
	...
	,	isAtLeastOneObjectReferencedByNoun[
				i o;
			for(i=0:i<number_matched:i++){
				o=(match_list-->i);
				if(o>0 && o has noun_ref) rtrue;
			}
			rfalse;
		]
	,	ext_chooseobjects[ obj c;
			...
			if(obj hasnt noun_ref) { !--objects referred to only by adjectives are not rated as highly
				if(self.isAtLeastOneObjectReferencedByNoun()==true) util.orParser.excludeObjFromDisambiguation(obj); !--if this object is only referenced by an adjective, and at least one other object was referenced by a noun, then just kill this option entirely, so it wont appear in disambiguation lists later
