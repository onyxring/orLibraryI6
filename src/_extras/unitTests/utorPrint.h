...
object -> fathernaru "Father Naru" has proper;
object -> gotie "Gotie";
...
orUnitTest "utorPrint" 
   with tests [; 
         ...
         [;
            actor=fathernaru; 
            noun=gotie; 
            orPrint("$actor(bless:blesses:blessed) $noun.");
            orPrint("$name:1(materialize:materializes:materialized) to stop $2.", spirit, scrooge);
            selfobj.narrative_tense=PAST_TENSE;
            orPrint("$actor(bless:blesses:blessed) $noun.");
            selfobj.narrative_tense=PRESENT_TENSE;
             
         ]
            [val count retval;
               count++; retval=retval+self.assertContains("Father Naru blesses Gotie.");
               count++; retval=retval+self.assertContains("Jacob materializes to stop Ebenezer.");
               count++; retval=retval+self.assertContains("Father Naru blessed Gotie.");
               return retval==count;              
            ]
